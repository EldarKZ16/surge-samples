...
class CreateAccountSimulation extends Simulation {
  ...
  val httpProtocol: HttpProtocolBuilder = http.baseUrl(scalaAppUrl).header("content-Type", "application/json")
  ...
  val scn: ScenarioBuilder = scenario("create user account")
    .exec(
      http("create account request")
        ...
        .check(jsonPath("$.accountNumber").saveAs("aggregateId")))
    .exec(session => {
    ...
    .exec(http("Get Bank account state").get("/bank-accounts/${aggregateId}").check(status.is(200), bodyString.saveAs("balance")))
    .exec(session => {
